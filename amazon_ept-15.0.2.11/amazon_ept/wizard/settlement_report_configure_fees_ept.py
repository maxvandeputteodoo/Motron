...
class SettlementReportConfigureFeesEPT(models.TransientModel):
    ...
    @staticmethod
    def update_transaction_line_vals(tax_id, analytic_account_id, values, analytic_tag_ids):
        """
        ...
        if analytic_tag_ids:
            values.update({'analytic_account_tag_ids': analytic_tag_ids})
        return values
    ...
    def configure_settlement_report_fees(self):
        ...
        for configure_line in self.settlement_report_configure_line_ids:
            ...
            analytic_tag_ids = configure_line.analytic_account_tag_ids.ids if \
                configure_line.analytic_account_tag_ids else False
            ...
            if amazon_code:
                ...
                values = self.update_transaction_line_vals(tax_id, analytic_account_id, values, analytic_tag_ids)
                line_id = self.get_amazon_transaction_line(seller_id, amazon_code, values)
                line_id.write({'account_id': account_id, 'analytic_account_id': analytic_account_id,
                               'tax_id': tax_id, 'analytic_account_tag_ids': analytic_tag_ids})
        return True
...
class SettlementReportConfigureFeesLineEPT(models.TransientModel):
    ...
    analytic_account_tag_ids = fields.Many2many('account.analytic.tag', relation='settlement_fees_analytic_tag_ept',
                                                string='Analytic Tags', help='Analytic Tags')
