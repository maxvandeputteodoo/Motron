                ...
                for marketplace in marketplace_ids:
                    ...
                    if instance_exist:
                        if marketplace.market_place_id not in ['A1805IZSGTT6HS', 'A2NODRKZP88ZB9']:
                            ...
                            other_eu_instance.append(instance_exist)
                        continue
                    vals = self.prepare_amazon_marketplace_vals(marketplace, warehouse_id)
                    if marketplace.market_place_id not in ['A1805IZSGTT6HS', 'A2NODRKZP88ZB9']:
                        fba_warehouse_id = self.get_fba_warehouse(view_location_id, lot_stock_id, \
                    ...
                    fba_warehouse_id.write({'resupply_wh_ids': [warehouse_id]})
                    amazon_instance_obj.create(vals)
                if other_eu_instance or other_eu_vals:
                    ...
                    if de_warehouse_id:
                        for instance in other_eu_instance:
                            instance.write({'fba_warehouse_id': de_warehouse_id.id})
