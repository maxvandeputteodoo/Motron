...
class SaleOrder(models.Model):
    ...
    def import_fba_pending_sales_order(self, seller, marketplaceids, updated_after_date):
        ...
        if response.get('error', False):
            if self._context.get('is_auto_process', False):
                log_rec = self.search_or_create_amz_log_ept('sale.order')
                log_rec.write({'log_lines': [(0, 0, {'message': response.get('error', {})})]})
                return True
    ...
    def search_or_create_amz_log_ept(self, model_id):
        """
        This method will find exist log record for sale order or create new log record.
        :return: common.log.book.ept() object
        """
        common_log_book_obj = self.env['common.log.book.ept']
        model_id = self.env['ir.model']._get(model_id).id
        log_rec = common_log_book_obj.search(
            [('module', '=', 'amazon_ept'), ('model_id', '=', model_id), ('res_id', '=', self.id)], limit=1)
        if not log_rec:
    ...
    def create_or_find_amazon_product(self, order_details, product_details, instance,
        ...
        if not amazon_product:
            odoo_product, skip_order = self.amz_search_or_create_odoo_product(
                instance, amazon_order_ref, fulfillment_by, order_details, skip_order, log_book)
            if not skip_order:
    ...
    def amz_create_order_lines(self, order, instance, order_details, dict_product_details):
        ...
        line_vals = {
            ...
            'order_qty': order_details.get('QuantityOrdered', 0.0),
            'price_unit': unit_price,
            'product_uom_qty': order_details.get('QuantityShipped', 0.0),
    ...
    def amz_create_sales_order(self, queue_order, log_book):
        ...
        for line in queue_lines:
            ...
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
            if response.get('error', False):
                common_log_line_ept.amazon_create_order_log_line(response.get('error'), model_id, False,
                                                                 amazon_order_ref, False, 'FBM', log_book)
                line.state = 'failed'
                continue
    ...
    def get_fbm_next_token_orders(self, seller, next_token):
        ...
        if response.get('error', False):
            if self._context.get('is_auto_process', False):
                log_rec = self.search_or_create_amz_log_ept('shipped.order.data.queue.ept')
                log_rec.write({'log_lines': [(0, 0, {'message': response.get('error', {})})]})
                return []
    ...
    def create_amazon_shipped_or_unshipped_order_vals(self, instance, partner, order):
        ...
        # Find and set B2B fiscal position in order if orders is Business (B2B) order
        if ordervals.get('is_business_order', False):
            ordervals = self.amz_set_b2b_fiscal_position_ept(ordervals, ordervals.get('is_business_order', False))
        return ordervals
    ...
    def amz_set_b2b_fiscal_position_ept(self, ordervals, is_business_order):
        """
        This method will help to find and set B2B fiscal position for amazon sale orders.
         :param ordervals: dict {}
         :param is_business_order: boolean (TRUE/False)
         :return : dict {}
        """
        order_vals = {
            'company_id': ordervals.get('company_id', False),
            'partner_id': ordervals.get('partner_id', False),
            'partner_invoice_id': ordervals.get('partner_invoice_id', False),
            'partner_shipping_id': ordervals.get('partner_shipping_id', False),
            'warehouse_id': ordervals.get('warehouse_id', False),
        }
        new_record = self.new(order_vals)
        new_record.with_context(is_b2b_amz_order=is_business_order).onchange_partner_shipping_id()
        new_order_vals = self._convert_to_write({name: new_record[name] for name in new_record._cache})
        fpos_id = new_order_vals.get('fiscal_position_id', False)
        if fpos_id and fpos_id != ordervals.get('fiscal_position_id', False):
            ordervals.update({'fiscal_position_id': fpos_id})
        return ordervals
