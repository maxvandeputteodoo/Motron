...
class FbmSaleOrderReportEpt(models.Model):
    ...
    def process_fbm_sale_order_file(self):
        ...
        for x in range(0, len(file_order_list), 50):
            ...
            if response.get('error', False):
                if self._context.get('is_auto_process', False):
                    common_log_line_obj.amazon_create_order_log_line(response.get('error', {}), model_id, self.id,
        ...
        self.process_prepare_unshipped_order_list_ept(unshipped_order_list, business_prime_dict, log_book)
        if not log_book.log_lines:
            log_book.unlink()
        self.write({'state': 'processed'})
        return True

    def process_prepare_unshipped_order_list_ept(self, unshipped_order_list, business_prime_dict, log_book):
        """
    ...
    def create_amazon_fbm_unshipped_order(self, instance, partner, order_ref, order_details, business_prime_dict):
        ...
        if ordervals.get('is_business_order', False):
            ordervals = sale_order_obj.amz_set_b2b_fiscal_position_ept(
                ordervals, ordervals.get('is_business_order', False))
