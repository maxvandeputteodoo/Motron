...
class AmazonReports(models.AbstractModel):
    ...
    def get_report(self):
        ...
        if self.report_document_id:
            if not self._context.get('amz_report_type', False) and self._description == 'Shipping Report':
                amz_report_type = '' if self.report_type == 'GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL' else \
    ...
    def amz_search_or_create_logs_ept(self, message):
        ...
        is_missing_fulfillment_center = self._context.get('is_missing_fulfillment_center', False)
        log_rec = common_log_book_obj.amazon_search_or_create_transaction_log('import', model_id, self.id)
        if (log_rec and log_rec.log_lines) and not is_missing_fulfillment_center:
